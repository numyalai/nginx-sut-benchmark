...
echo "Creating a new sut instance with name: $SUTinstanceName"
gcloud compute instances create $SUTinstanceName --source-instance-template="demobench" --zone="europe-west3-c"
sleep 40s
...
gcloud compute ssh $SUTinstanceName --zone=europe-west3-c --command="sudo apt update && sudo apt upgrade -y"
gcloud compute ssh $SUTinstanceName --zone europe-west3-c -- 'sudo apt install -y -q nginx'
...
gcloud compute ssh $SUTinstanceName --zone europe-west3-c -- 'sudo cat /etc/nginx/nginx.conf'

gcloud compute ssh $SUTinstanceName --zone=europe-west3-c --command="whoami"

# Create a directory for binary files inside /etc/nginx
